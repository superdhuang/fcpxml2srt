...
#                       |<-postOffset->|                                     
    ...
    for element in root.iter():
        if element.tag in ["ref-clip", "clip", "asset-clip", "transition"]:
            parent_offset = convert_xml_t(element.attrib.get('offset', None))
            parent_start = convert_xml_t(element.attrib.get('start',None))

            for marker in element.iter('marker'):
                marker_id = id(marker)
                # 檢查是否已經處理過此 marker
                if marker_id not in processed_markers:
                    offset = parent_offset - parent_start + convert_xml_t(marker.attrib.get('start', None))
                    duration = convert_xml_t(marker.attrib.get('posterOffset'))
                    end = offset + duration
                    text = marker.attrib.get('value')
#                    data.append((offset, end, text, 'stdMarker'))
                if 'completed' in marker.attrib:
                    data.append((offset, end, text, 'todo'))
                else:
                    data.append((offset, end, text, 'stdMarker'))

            for marker in element.iter('chapter-marker'):
                marker_id = id(marker)
                # 檢查是否已經處理過此 marker
                if marker_id not in processed_markers:
                    offset = parent_offset - parent_start + convert_xml_t(marker.attrib.get('start', None))
                    ...
                    text = marker.attrib.get('value')
                    data.append((offset, end, text, 'chMarker'))
